import { addMessage, archiveClosedRoomCode, archiveMatch, archivedMatchesForToken, chroniclesForRoom, createRoom, joinRoom, kintsugiForRoom, messagesForRoom, playerByHistoryToken, playerByToken, playersForRoom, revokeActiveToken, roomByCode, saveChronicleAnswer, saveKintsugiFragment, saveKintsugiPortrait, savePlayerAvatar, saveState, type ChroniclePublicProfile, type KintsugiPublic } from "@/lib/game-store";
...
function publicState(state: GameState, playerId: string, messages: unknown[], chronicles: ChroniclePublicProfile[] = [], kintsugi?: KintsugiPublic) {
...
export async function GET(request: Request) {
  try {
    ...
    const [messages, chronicles, kintsugi] = await Promise.all([messagesForRoom(room.id), chroniclesForRoom(room.id), kintsugiForRoom(room.id, player)]);
    return Response.json({ state: publicState(state, player.id, messages, chronicles, kintsugi) });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    } else if (action === "kintsugi-fragment") {
      if (player.seat !== 0) throw new Error("Only Sky can name these fragments.");
      const players = await playersForRoom(room.id); const subject = players.find((entry) => entry.seat === 1);
      const questionId = Number(body.questionId); const word = String(body.word ?? "").trim().slice(0, 28);
      if (!subject || !chronicleQuestionIds.has(questionId)) throw new Error("That Chronicle page could not be found.");
      if (!word || /\s/.test(word)) return Response.json({ error: "Each fragment must contain exactly one word." }, { status: 400 });
      await saveKintsugiFragment(room.id, subject.id, questionId, word);
    } else if (action === "kintsugi-portrait") {
      if (player.seat !== 0) throw new Error("Only Sky can write this legend.");
      const players = await playersForRoom(room.id); const subject = players.find((entry) => entry.seat === 1);
      if (!subject) throw new Error("There is no Chronicle to write yet.");
      const portrait = String(body.portrait ?? "").trim().slice(0, 12000); const seal = body.seal === true;
      const chronicles = await chroniclesForRoom(room.id); const subjectChronicle = chronicles.find((entry) => entry.playerId === subject.id);
      if (!portrait) return Response.json({ error: "Write their legend before sealing it." }, { status: 400 });
      if (seal && Object.keys(subjectChronicle?.answers ?? {}).length < chronicleQuestionIds.size) return Response.json({ error: "Their legend stays locked until every Chronicle question is answered." }, { status: 400 });
      await saveKintsugiPortrait(room.id, subject.id, player.id, portrait, seal);
    ...
    const [messages, chronicles, kintsugi] = await Promise.all([messagesForRoom(room.id), chroniclesForRoom(room.id), kintsugiForRoom(room.id, player)]);
    return Response.json({ historyToken: player.history_token, chronicleAwarded, state: publicState(state, player.id, messages, chronicles, kintsugi) });
  } catch (error) {
