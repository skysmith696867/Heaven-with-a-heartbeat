...
export const kintsugiFragments = sqliteTable("kintsugi_fragments", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  roomId: text("room_id").notNull().references(() => rooms.id, { onDelete: "cascade" }),
  subjectPlayerId: text("subject_player_id").notNull().references(() => players.id, { onDelete: "cascade" }),
  questionId: integer("question_id").notNull(),
  word: text("word").notNull(),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("kintsugi_fragment_subject_question_unique").on(table.subjectPlayerId, table.questionId)]);

export const kintsugiPortraits = sqliteTable("kintsugi_portraits", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  roomId: text("room_id").notNull().references(() => rooms.id, { onDelete: "cascade" }),
  subjectPlayerId: text("subject_player_id").notNull().references(() => players.id, { onDelete: "cascade" }),
  authorPlayerId: text("author_player_id").notNull().references(() => players.id, { onDelete: "cascade" }),
  body: text("body").notNull().default(""),
  sealedAt: text("sealed_at"),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("kintsugi_portrait_subject_unique").on(table.subjectPlayerId)]);
